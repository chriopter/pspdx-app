...
/* The popups under the gear: the catalogs this console reads, the two
   ways a .pspdx comes in directly, and the resets. */
enum sub { SUB_NONE, SUB_CATALOGS, SUB_ADD, SUB_RESET };
void sub_open(enum sub which);

/* 1 while the options or a popup stand over the browser. */
int menu_shown(void);
...
/* One frame's keys while the options or a popup stand: up and down walk
   the rows, O closes, X takes the row, and the keys the menu names do
   their row's thing from inside it. Returns 1 while either stood, when the
